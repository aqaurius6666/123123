...
export const CheckAuth = async (req, res, next) => {
	try {
		...
		findUserByUsername(authInfo.certificateInfo.username).then((userInfo) => {
			const decryptedEmailSignature = SymmetricDecrypt(
				emailSignature,
				SYMETRIC_SECRET
			);
			const decryptedGGSignature = SymmetricDecrypt(
				googleSignature,
				SYMETRIC_SECRET
			);

			const isEmailAuthenEnable = userInfo.isVerifyEmail,
				isGoogleAuthenEnable = userInfo.token2FA.length > 0;

			if (isEmailAuthenEnable) {
				if (!emailSignature) {
					res.json(unauthorized('AUTH.EMAIL_AUTHENTICATION.INVALID'));
					return;
				}

				console.log(JSON.parse(decryptedEmailSignature), userInfo);

				if (
					JSON.parse(decryptedEmailSignature).userId !==
				...
				if (emailSignature && !JSON.parse(decryptedEmailSignature).isVerified) {
					res.json(unauthorized('AUTH.EMAIL_AUTHENTICATION.INVALID'));
					return;
				}
			}

			if (isGoogleAuthenEnable) {
				if (!googleSignature) {
					res.json(unauthorized('AUTH.GG_AUTHENTICATION.INVALID'));
					return;
				}

				if (
					JSON.parse(decryptedGGSignature).userId !==
					userInfo._id.toString()
				) {
					res.json(unauthorized('AUTH.EMAIL_AUTHENTICATION.INVALID'));
					return;
				...
				if (googleSignature && !JSON.parse(decryptedGGSignature).isVerified) {
					res.json(unauthorized('AUTH.GG_AUTHENTICATION.INVALID'));
					return;
				}
			}
