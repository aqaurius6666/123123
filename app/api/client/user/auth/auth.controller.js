	...
	try {
		...
		if (!username) {
			...
			const user = await User.findOne({ username })
				.select(
					'username lastName firstName fullName publicKey encryptedPrivateKey isActive token2FA isVerifyEmail email'
				)
				.lean();
			if (!user) throw new Error('AUTH.ERROR.USER_NOT_FOUND');
			if (!user.isActive) throw new Error('AUTH.ERROR.USER_NOT_ACTIVE');
			if (user && user.isActive) {
				return res.json(
					success({
						...user,
						isGoogleAuthenEnable:
							typeof user.token2FA === 'string' && user.token2FA.length > 0,
						isEmailAuthenEnable: user.isVerifyEmail,
						token2FA: undefined,
...
api.post('/auth/ping', CheckAuth, async (req, res) => {
	try {
		const user = await User.findOne({
			...
			.populate(['role'])
			.select(
				'username lastName firstName fullName publicKey encryptedPrivateKey role '
			);
		if (user) {
